...
class BaseResource(Resource):
    ...
    def query(self):
        ...
        return self.service_klass.model_class.query
    ...
    def limit_query(self, query, **kwargs):
        ...
        if not user_context and request.environ.get('public_view'):
            return query
    ...
    def put(self, obj_id=None, resource_name=None):
        ...
        resp_serializer = self.serializers.get("response")
        if resource_name:
            func = getattr(self, resource_name, None)
            if not func:
                raise abort(409, 'requested resource does not exist')

            serializer = self.serializers.get(resource_name, serializer)
            resp_serializer = self.serializers.get(f'{resource_name}_response', resp_serializer)
        ...
        try:
            ...
            return abort(409, e.messages)
        user_context = request.environ.get("user_context")
        resp = func(obj_id=obj_id, data=validated_data, user_context=user_context)
        return resp_serializer().dump(resp)
